...
class FileUploadUseCaseTest {
    ...
    class MultipartFileUploadTest {
        ...
        void shouldUploadWithDefaultPath() {
            ...
            given(fileResourceFactory.fromMultipartFile(file, DEFAULT_PATH))
                    .willReturn(mockFileResource);
            given(repository.store(mockFileResource))
                    .willReturn(GENERATED_FILE_ID);
            ...
            then(fileResourceFactory).should().fromMultipartFile(file, DEFAULT_PATH);
            then(repository).should().store(mockFileResource);
        ...
        void shouldUploadWithCustomPath() {
            ...
            given(fileResourceFactory.fromMultipartFile(file, CUSTOM_PATH))
                    .willReturn(mockFileResource);
            given(repository.store(mockFileResource))
                    .willReturn(GENERATED_FILE_ID);
            ...
            then(fileResourceFactory).should().fromMultipartFile(file, CUSTOM_PATH);
            then(repository).should().store(mockFileResource);
        }
        ...
        void shouldRejectInvalidMultipartFile(String testCase, MultipartFile invalidFile, FileUploadErrorCode expectedErrorCode) {
            ...
            then(fileResourceFactory).shouldHaveNoInteractions();
    ...
    class FileUploadTest {
        ...
        void shouldUploadFile() {
            ...
            // FileResourceFactory에서 정적 메서드를 인스턴스 메서드로 변경해야 함
            // fromFile 메서드를 인스턴스 메서드로 변경
            given(fileResourceFactory.fromFile(file, DEFAULT_PATH))
                    .willReturn(mockFileResource);
            given(repository.store(mockFileResource))
                    .willReturn(GENERATED_FILE_ID);
            ...
            then(fileResourceFactory).should().fromFile(file, DEFAULT_PATH);
            then(repository).should().store(mockFileResource);
        }
    ...
    class ExceptionHandlingTest {
        ...
        void shouldPropagateFactoryExceptions() {
            ...
            given(fileResourceFactory.fromMultipartFile(any(), any()))
                    .willThrow(factoryEx);
